...
/* option menu is not used, so that annotate it.
   because context menu will use menu key
    @Override
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if(keyCode == KeyEvent.KEYCODE_MENU ){
            switch(mViewPager.getCurrentItem()){
                //  0: context menu of Bookmark
                case 0:
                {
                    View currentView = getCurrentFocus();
                    if (currentView != null  &&  R.id.grid != currentView.getId()) {
                        openContextMenu(currentView);
                    }
                }
                    break;
                //  1: context menu of History        
                case 1:
                case 2:
                default:
                    break;
