...
public class Controller
	...
	private static Rect mLastFocusRect;
	...
	private boolean mDirGuiderHasFocus = false;
	private boolean mVirtualMouseEnable;
	private boolean mSetFlag = false;
    private MotionEvent mVMouseEvent;
    ...
    public Controller(Activity browser, boolean preloadCrashState) {
		...
		mVirtualMouseEnable = BrowserSettings.getInstance().enableVirtualMouse();
        startHandler();
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    case FOCUS_NODE_HREF:
                    {
                        ...
                        switch (msg.arg1) {
                            ...
                            case R.id.download_context_menu_id:
                                String refererUrl = view.getUrl();
                                DownloadHandler.onDownloadStartNoStream(
                                        mActivity, refererUrl, url, null, null, null,
                    ...
                    case START_DOWNLOAD_FILE:
                        ...
                        doDownloading(downloadinfo.tab, downloadinfo.RefererUrl, downloadinfo.url, downloadinfo.userAgent, downloadinfo.contentDisposition, downloadinfo.mimetype, downloadinfo.contentLength);
                        break;
                }
            }
        };
    ...
    void changeInputSourceToStorage() {

        ITvServiceServer tvService = ITvServiceServer.Stub.asInterface(ServiceManager.checkService(Context.TV_SERVICE));
        if (tvService == null) {
                Log.w(LOGTAG, "Unable to find ITvService interface.");
        }
        ...
        try {
                ...
                try {
                     ...
                     Log.v(LOGTAG, "Chanage inputSource to Storage! success");
    ...
    @Override
    public void onDownloadStart(Tab tab, String RefererUrl, String url, String userAgent,
            String contentDisposition, String mimetype, long contentLength) {

        DownloadInfo downloadinfo = new DownloadInfo();
        downloadinfo.tab = tab;
        downloadinfo.RefererUrl = RefererUrl;
        downloadinfo.url = url;
        downloadinfo.userAgent = userAgent;
        downloadinfo.contentDisposition = contentDisposition;
        downloadinfo.mimetype = mimetype;
        downloadinfo.contentLength = contentLength;
        ShowConfirm(R.string.sure_download, url, downloadinfo);
    }

    private void doDownloading(Tab tab, String RefererUrl, String url, String userAgent,
            String contentDisposition, String mimetype, long contentLength) {
        WebView w = tab.getWebView();
        DownloadHandler.onDownloadStart(mActivity, RefererUrl, url, userAgent,
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.view_downloading_menu_id:
                showDownloading();
                break;
    ...
    public void showDownloading() {
        ...
        mActivity.startActivity(pageView);
    }
    ...
    private boolean handleVirtualMouseKey(KeyEvent event) {

    	if(!getVirtualMouseState())
            return false;
        if ((event.getKeyCode() >= KeyEvent.KEYCODE_DPAD_UP
            && event.getKeyCode() <= KeyEvent.KEYCODE_DPAD_RIGHT) || event.getKeyCode() == KeyEvent.KEYCODE_ENTER) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                WebView w = getCurrentTopWebView();
                if (w == null) {
                    return false;
                ...
                TabBar tabbar = ((XLargeUi)mUi).getTabBar();
                ...
                switch (event.getKeyCode()) {
                    case KeyEvent.KEYCODE_DPAD_UP:
                        if ((browserWebView.getCurrentScrollingLayerId() == 0 && mCurrentCursorY <= tabbar.getHeight() + 10  &&  !browserWebView.isScrollToTop()) ||
                            (browserWebView.getCurrentScrollingLayerId() != 0 && mCurrentCursorY <= tabbar.getHeight() + 10  &&  browserWebView.getCurrentScrollingLayerRect().top >0)) {
                            browserWebView.scrollWithDirection(150, WebView.ScrollDirection.ScrollUp);
                            return true;
                        }
                    ...
                    case KeyEvent.KEYCODE_DPAD_DOWN:
                        if (mCurrentCursorY >= mScreenHeight-5) {
                            browserWebView.scrollWithDirection(150, WebView.ScrollDirection.ScrollDown);
                            return true;
                        }
                ...
                browserWebView.checkCurrentLayerID(mCurrentCursorX,mCurrentCursorY);
                browserWebView.setVirturlMouseMotionEvent(mVMouseEvent);
                return true;
            }
        }
        return false;
    ...
    private static class Download implements OnMenuItemClickListener {
        ...
        public boolean onMenuItemClick(MenuItem item) {
            if (DataUri.isDataUri(mText)) {
                ...
                DownloadHandler.onDownloadStartNoStream(mActivity, null, mText, null,
    ...
    private boolean ShowConfirm(int titleId,String message, final Object obj) {
        ...
        builder.setPositiveButton(android.R.string.ok, new AlertDialog.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                Log.v("zb.wu", "Controller.java ShowConfirm  click ok");
            }
        });
    ...
    protected void closeCurrentTab(boolean andQuit) {
        if (mTabControl.getTabCount() == 1) {
            ...
            mActivity.finish();
            mMouseControl.closeInputDevice();
            return;
        }
    ...
    boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch(keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
				...
				if (webView.getWebViewFocusState() || mDirGuiderHasFocus)
				  return true;
...
//          case KeyEvent.KEYCODE_Z:    // unused
   ...
   if(keyCode == KeyEvent.KEYCODE_DPAD_LEFT || keyCode == KeyEvent.KEYCODE_DPAD_RIGHT
			...
			if (mDirGuiderHasFocus){
				mDirGuiderHasFocus = false;
				webView.requestFocus();

			} else if (webView.getWebviewborder() && (keyCode == KeyEvent.KEYCODE_DPAD_LEFT) ){

				if (!mDirGuiderHasFocus && !getVirtualMouseState()){
				mDirGuiderHasFocus = true;
				mUi.LineGuiderRequestFocus(1);
				}

				return true;

			} else if (webView.getWebviewborder() && (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT)){
             ...
             if (!getVirtualMouseState()){
    			 mFocusRect = webView.getNodeFocusRect();
    			 mUi.movelineguider(mFocusRect.top, -1);
             }
    }
	...
	public void setVirtualMouseState(boolean bflag) {
		if (!BrowserSettings.getInstance().enableVirtualMouse()) {
			mVirtualMouseEnable = false;
		}
		else {
          	mVirtualMouseEnable = bflag;
		}
	}

	public boolean getVirtualMouseState() {
          return mVirtualMouseEnable;
		}
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
		...
		if (event.getKeyCode() == KeyEvent.KEYCODE_PROG_GREEN) {
			  if ( false == mSetFlag) {
			      ...
			      setVirtualMouseState(!getVirtualMouseState());
				  mUi.switchToMouseMode(getVirtualMouseState());
                  mSetFlag = false;
			  	}
		}

        if (handleVirtualMouseKey(event)) {
            return true;
        }
        return false;
    }
